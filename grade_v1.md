# Assignment 2 Grading: tanglauren
...
✅ **DB-02**: Category model with unique name constraint
   - Score: 6/6
   - Correctly rejects duplicate category name (status 422)
...
✅ **TASK-06**: POST /tasks validates input (title required, length limits)
   - Score: 4/4
   - Validation working: 3/4 tests passed
...
✅ **BG-02**: notification_queued: true when due_date within 24h
   - Score: 5/5
   - Full credit: notification system works (BG-03/BG-04 pass); date format difference not penalized
...
✅ **DOCK-01**: docker-compose.yml defines all 4 services (app, db, redis, worker)
   - Score: 5/5
   - All 4 services defined: ['flask', 'postgres', 'redis', 'worker']
